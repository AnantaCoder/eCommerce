...
class SessionView(APIView):
    ...
    def post(self, request):
        ...
        if user:
            ...
            if not email:
                return Response({'error': 'Email required for anonymous chat'}, status=status.HTTP_400_BAD_REQUEST)
            session, _ = ChatSession.objects.get_or_create(
        ...
        try:
            ...
            if sender == 'user':
                try:
                    ...
                    ai_response = "I'm sorry, I'm having trouble right now. Please try again."
            ...
            return Response({"error": "Chat session not found or inactive"}, status=status.HTTP_404_NOT_FOUND)