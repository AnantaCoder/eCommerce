...
function ManageItems() {
  ...
  const { items, loading, totalPages } = useSelector((state) => state.store)
  ...
  useEffect(() => {
    ...
    if (user?.id) {
      dispatch(fetchSellerItems({ sellerId: user.id, page }))
    }
  }, [dispatch, user, page])
        ...
        <div className="bg-gray-800 rounded-2xl p-8 max-w-md w-full border border-gray-700">
          <div className="text-center">
            ...
            <p className="text-gray-400">
              You need to be registered as a seller to manage items.
            </p>
          </div>
        </div>
      </div>
          ...
          <>
                  ...
                  {editingItem === item.id ? (
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2 ml-4 flex-shrink-0">
                        <button
                          onClick={() => setAnalyzingItemId(item.id)}
                      ...
                      {analyzingItemId === item.id && (
                        <AnalyzeComponent
                          itemId={item.id}
                          onClose={() => setAnalyzingItemId(null)}
                        />
                      )}
                    </div>
                  )}
            ...
            <div className="flex justify-center items-center space-x-4 mt-6">
              <button
                ...
                className="px-4 py-2 bg-gray-700 text-white rounded-xl disabled:opacity-50 transition-colors"
              >
                Previous
              </button>
